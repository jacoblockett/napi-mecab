...
#if defined(_WIN32) && !defined(__CYGWIN__)
extern HINSTANCE DllInstanceJp;
#endif
...
int load_request_type(const Param &param) {
  int request_type = MECAB_JP_ONE_BEST;

  if (param.get<bool>("allocate-sentence")) {
    request_type |= MECAB_JP_ALLOCATE_SENTENCE;
  }

  if (param.get<bool>("partial")) {
    request_type |= MECAB_JP_PARTIAL;
  }

  if (param.get<bool>("all-morphs")) {
    request_type |= MECAB_JP_ALL_MORPHS;
  }

  if (param.get<bool>("marginal")) {
    request_type |= MECAB_JP_MARGINAL_PROB;
  }

  const int nbest = param.get<int>("nbest");
  if (nbest >= 2) {
    request_type |= MECAB_JP_NBEST;
  }
