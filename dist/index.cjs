...
const dictionaries = {
	jp: path.join(__dirname$1, "..", "dict", "jp-dict"),
	ko: path.join(__dirname$1, "..", "dict", "ko-dict")
};
const JP = "jp"; // UniDic
...
class MeCab {
	...
	constructor(opts) {
		...
		if (!fs.existsSync(this.#dictPath)) throw new Error(`"${this.#dictPath}" doesn't exist.`)

		const expectedFiles = ["char.bin", "dicrc", "matrix.bin", "sys.dic", "unk.dic"];
		try {
			const foundFiles = fs.readdirSync(this.#dictPath);
